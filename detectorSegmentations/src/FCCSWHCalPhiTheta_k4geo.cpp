// still to do:
//   thread-safety
//   don't use string fields to decoder (for lar too!)
...
{
  if (m_layerInfo.empty()) {
    m_layerInfo = initLayerInfo();
  }
  return m_layerInfo.at (layer);
}
                                              ...
                                              const unsigned int layer) const
{
  ...
  { double theta = m_offsetTheta+ibin*m_gridSizeTheta;
    return li.radius / std::tan(theta);
  };

  // find theta bins that fit within the given layer
  int ibin = positionToBin(0.02, m_gridSizeTheta, m_offsetTheta);
  for (; ; ++ibin)
  {
  ...
  dd4hep::printout(dd4hep::DEBUG, "FCCSWHCalPhiTheta_k4geo","Number of cells in layer %d: %d", layer, li.thetaBins.size());
  //for(auto bin : li.thetaBins) dd4hep::printout(dd4hep::DEBUG, "FCCSWHCalPhiTheta_k4geo","Layer %d cell theta bin: %d, edges: %.2f - %.2f cm", layer, bin, li.cellEdges[bin]);
}
...
CellID FCCSWHCalPhiTheta_k4geo::cellID(const Vector3D& /* localPosition */, const Vector3D& globalPosition,
  ...
  for(auto bin : li.thetaBins)
  {
    ...
    {
      decoder()->set(cID, m_thetaID, bin);
      decoder()->set(cID, m_phiID, positionToBin(lPhi, 2 * M_PI / (double)m_phiBins, m_offsetPhi));
      return cID;
    }
  }
  ...
  decoder()->set(cID, m_thetaID, positionToBin(lTheta, m_gridSizeTheta, m_offsetTheta));
  decoder()->set(cID, m_phiID, positionToBin(lPhi, 2 * M_PI / (double)m_phiBins, m_offsetPhi));
  return cID;
}
...
double FCCSWHCalPhiTheta_k4geo::phi(const CellID& cID) const {
  CellID phiValue = decoder()->get(cID, m_phiID);
...
std::vector<uint64_t> FCCSWHCalPhiTheta_k4geo::neighbours(const CellID& cID, bool aDiagonal) const {
   ...
   int currentLayerId = decoder()->get(cID,m_layerID);
   int currentCellThetaBin = decoder()->get(cID,m_thetaID);
   ...
   {
     CellID nID = cID ;
     decoder()->set(nID,m_thetaID,currentCellThetaBin - 1);
   ...
   if(m_detLayout == 0)
   {
     ...
     if(currentLayerId > minLayerId)
     {
       ...
       decoder()->set(nID,m_layerID,prevLayerId);

       decoder()->set(nID,m_thetaID,currentCellThetaBin);
       ...
       if(theta(cID) < M_PI/2. && currentCellThetaBin > prevLI.thetaBins.front() )
       {
         ...
         if(aDiagonal && currentCellThetaBin > (prevLI.thetaBins.front()+1))
         {
           ...
           if(zmin <= currentCellZmax)
           {
             ...
             decoder()->set(nID,m_thetaID,currentCellThetaBin - 2);
     ...
     if(currentLayerId < maxLayerId)
     {
       ...
       decoder()->set(nID,m_layerID,nextLayerId);
     ...
     if(m_offsetZ.size() > 1)
     {
       ...
       if(EndcapPart == 0 && currentCellThetaBin == minCellThetaBin )
       {
         ...
         for(int part2layerId = minLayerIdEndcap[1]; part2layerId <= maxLayerIdEndcap[1]; part2layerId++)
         {
           ...
           {
             CellID nID = cID ;
             decoder()->set(nID,m_layerID,part2layerId);
             decoder()->set(nID,m_thetaID,maxCellThetaBin);
       ...
       for(uint i_section = 1; i_section < (m_offsetZ.size()-1); i_section++)
       {
         ...
         if(currentCellThetaBin == maxCellThetaBin)
         {
           ...
           for(int prevPartLayerId = minLayerIdEndcap[i_section-1]; prevPartLayerId <= maxLayerIdEndcap[i_section-1]; prevPartLayerId++)
           {
             ...
             {
               CellID nID = cID ;
               decoder()->set(nID,m_layerID,prevPartLayerId);
               decoder()->set(nID,m_thetaID,minCellThetaBin);
         ...
         if(currentCellThetaBin == minCellThetaBin)
         {
           ...
           for(int nextPartLayerId = minLayerIdEndcap[i_section+1]; nextPartLayerId <= maxLayerIdEndcap[i_section+1]; nextPartLayerId++)
           {
             ...
             {
               CellID nID = cID ;
               decoder()->set(nID,m_layerID,nextPartLayerId);
               decoder()->set(nID,m_thetaID,maxCellThetaBin);
   ...
   for(auto nID : cellNeighboursCopy)
   {
     ...
     decoder()->set(newID,m_phiID, (decoder()->get(nID,m_phiID) == 0) ? m_phiBins - 1 : decoder()->get(nID,m_phiID) - 1);
     ...
     decoder()->set(newID,m_phiID, (decoder()->get(nID,m_phiID) == (m_phiBins - 1)) ? 0 : decoder()->get(nID,m_phiID) + 1);
     cellNeighbours.push_back(newID);
   }
   ...
   CellID nID = cID ;
   // previous: if the current is 0 then previous is the last bin (id = m_phiBins - 1) else current - 1
   decoder()->set(nID,m_phiID, (decoder()->get(cID,m_phiID) == 0) ? m_phiBins - 1 : decoder()->get(cID,m_phiID) - 1);
   cellNeighbours.push_back(nID);
   // next: if the current is the last bin (id = m_phiBins - 1) then the next is the first bin (id = 0) else current + 1
   decoder()->set(nID,m_phiID, (decoder()->get(cID,m_phiID) == (m_phiBins - 1)) ? 0 : decoder()->get(cID,m_phiID) + 1);
   cellNeighbours.push_back(nID);
...
std::array<double, 2> FCCSWHCalPhiTheta_k4geo::cellTheta(const CellID& cID) const {
  ...
  // get the cell index
  int idx = decoder()->get(cID, m_thetaID);
  // get the layer index
  uint layer = decoder()->get(cID,m_layerID);
